...
const Navbar: React.FC = () => {
    ...
    return (
        <>
            <nav className="fixed top-0 left-0 w-full z-[100] h-[72px] bg-[#0C0C0C]/85 backdrop-blur-[12px] border-b border-[#1C1C1C]">
                <div className="container h-full flex items-center justify-between">
                    <Link to="/" className="flex items-center space-x-3 group">
                        ...
                        <span className="text-[24px] font-[SpaceGrotesk] font-bold text-white tracking-widest uppercase">
                            FRAIMIIX STUDIOS
                        </span>
                    </Link>
                    ...
                    {/* Mobile Menu Toggle */}
                    <button
                        className="md:hidden text-[#F0F0F0] z-[110] relative p-2"
            ...
            {/* Mobile Menu Overlay */}
            <div
                className={`fixed inset-0 bg-[#0C0C0C] z-[90] md:hidden transition-all duration-500 ease-in-out ${isOpen ? 'opacity-100 pointer-events-auto visible' : 'opacity-0 pointer-events-none invisible'
                    }`}
            >
